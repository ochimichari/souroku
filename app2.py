...
with st.expander("🎙️ 新しい練習音声を Gemini AI で文字起こしする"):
    ...
    if uploaded_file and session_name and st.button("AI音声認識を実行"):
        ...
        try:
            ...
            正確な時間（分:秒）付きで文字起こし・抽出をしてください。
            フォーマットは必ず `[分:秒] 発言内容` としてください。
            
            条件：
            ・楽器の演奏音やメトロノームの音などは完全に無視してください。
            ・指揮者が演奏を止めて指示を出している部分を中心に抽出してください。

            【最重要：過去の誤認識補正リスト】
            {dict_text if dict_text else "（まだ登録はありません）"}
            """
            
            # 504エラー対策（タイムアウト5分延長設定）
            ...
            # タイムライン描画用のダミー枠を生成
            color_lines = [f"[{s//60:02d}:{s%60:02d}] MUSIC" for s in range(3600)]
            ...
            st.success("抽出とGitHubへの自動保存が完了しました！")
...
# 💡 上の【後半の1】の直後に、インデント（左の空白）を崩さずにそのまま続けてください。
...
if folders:
    ...
    html_lines = [
        ...
        "function parseColor(r){r.split('\\n').forEach(l=>{const t=l.trim();if(!t||!t.includes(' '))return;const p=t.split(' ');try{const[m,s]=p.replace('[','').replace(']','').split(':').map(Number);colorData[m*60+s]=p.trim();}catch(e){}})}",
        "function parseText(r){editor.innerHTML='';textTimestamps=[];let idx=0;r.split('\\n').forEach(l=>{const t=l.trim();if(!t)return;const d=document.createElement('div');d.className='line';d.innerText=t;d.id=`line-${idx}`;let s=-1;if(t.startsWith('[')&&t.includes(']')){try{const[m,sec]=t.split(']').replace('[','').split(':').map(Number);s=m*60+sec;}catch(e){}}textTimestamps.push(s);d.addEventListener('click',()=>{if(!isEditing&&s!==-1){audio.currentTime=s;audio.play();}});editor.appendChild(d);idx++;})}",

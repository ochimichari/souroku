...
if folders:
    ...
    # HTML/CSS/JavaScriptを完全に一体化して定義
    html_code = f"""
    <!DOCTYPE html>
    <html lang="ja">
    <head>
        ...
        <style>
            * {{ box-sizing: border-box; }}
            body {{ font-family: -apple-system, sans-serif; margin: 0; padding: 0; background: #0c0e12; color: #f0f2f5; display: flex; justify-content: center; }}
    ...
    # 💡 上の前半の html_code のすぐ直後に、インデント（左のスペース）を崩さずに続けてください
    html_code += f"""
    <script>
        ...
        function parseColor(raw) {{
            raw.split('\\n').forEach(line => {{
                ...
                try {{
                    const [m, s] = parts.replace('[', '').replace(']', '').split(':').map(Number);
                    colorData[m * 60 + s] = parts.trim();
                }} catch(e) {{}}
            }});
        }}
        ...
        function parseText(raw) {{
            ...
            raw.split('\\n').forEach(line => {{
                ...
                if (trimmed.startsWith('[') && trimmed.includes(']')) {{
                    try {{
                        const [m, s] = trimmed.split(']').replace('[', '').split(':').map(Number);
                        seconds = m * 60 + s;
                    }} catch(e) {{}}
                }}
        ...
        audio.addEventListener('timeupdate', () => {{
            ...
            for (let s = 0; s < totalDuration; s++) {{
                ...
                ctx.fillStyle = status === "SPEECH" ? "#cc4a5a" : (status === "MUSIC" ? "#00a2cc" : "#2a3247");
